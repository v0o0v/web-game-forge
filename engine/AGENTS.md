...
- **핵심 킷 4종(matterkit·screenfx·lightingkit·pathkit)은 선택적**이다. 미사용 게임에 부담 0 — 필요할 때만 `index.html`에 phaser 다음으로 추가한다.
...
### Internal
- 각 킷 파일은 대응 스킬과 1:1 — `engine/matterkit.js`↔`skills/matter-physics/`, `screenfx.js`↔`screen-fx/`, `lightingkit.js`↔`lighting-mood/`, `pathkit.js`↔`path-motion/`.
