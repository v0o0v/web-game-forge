# Phaser 4 레퍼런스 — Loading Assets

> **출처:** Phaser 공식 스킬 [`phaserjs/phaser/skills/loading-assets`](https://github.com/phaserjs/phaser/tree/master/skills/loading-assets) · MIT License.
> **대상 버전:** 본 플러그인 `engine/phaser.min.js` = **Phaser 4.1.0**.
> **이 문서를 읽어야 할 때:** preload, this.load, asset loading, spritesheet, atlas, load progress.
> **한글 라우팅·요약:** [INDEX.md](./INDEX.md)
>
> ⚠ 아래 본문은 Phaser 공식 영문 레퍼런스를 그대로 벤더링한 것이다(식별자·API 정확도 보존 목적). 새 코드는 반드시 **Phaser 4 API**로 작성한다. v3↔v4 차이 요약은 [INDEX.md](./INDEX.md) 와 [v3-to-v4-migration.md](./v3-to-v4-migration.md) 참고.
...
**Related skills:** ./game-setup-and-config.md, ./scenes.md, ./sprites-and-images.md
...
All `this.load` methods accept positional arguments or a single config object. They also accept an array of config objects to batch-load multiple files of the same type. See `./INDEX.md` for the complete parameter table.
...
Cache sub-types: `this.cache.text`, `this.cache.json`, `this.cache.audio`, `this.cache.binary`, `this.cache.shader`, `this.cache.xml`. See `./INDEX.md` for the full Cache API.

## Source File Map

See `./INDEX.md` for the complete source file map and Cache API reference.
