...
> - **톤·이야기·캐릭터·대사:** [`story-architect`](../wgf-story-architect/SKILL.md)
> - **습득·사용하는 모든 것:** [`item-architect`](../wgf-item-architect/SKILL.md)
...
> 그걸로 완결된다. 아주 작은/레트로 게임은 [`chip-sound`](../wgf-chip-sound/SKILL.md)(ChipAudio 8비트, T0)로 충분하다. 적응형
...
- **S1에서 무드 토큰 + 복잡도 티어(0~3)를 못 박는다.** T0(chip)이면 인터뷰 대부분 건너뛰고 [`chip-sound`](../wgf-chip-sound/SKILL.md)로 — **단순해도 된다고 적극 안내**.
...
- **라이브 웹 리서치(WebSearch/WebFetch):** 내장 원칙은 광범위 웹 리서치(`.omc/research/sound-research-dossier.md`)를 작은 웹게임용으로 정리한 1차 라이브러리이니 **항상 먼저 적용**. 그 위에, 특정 장르·레퍼런스 작품의 *사운드 결*이 필요하면 그 장르 음향 관습·합성 기법을 능동 리서치해 보강한다. **IP 안전 가드**: 합성 기법·음악 구조만 차용, 특정 곡 멜로디/진행+멜로디 결합은 오리지널 재구성([`ip-license-guard`](../wgf-ip-license-guard/SKILL.md)).
...
- **연출 정합:** 획득/타격 연출은 [`juice-fx`](../wgf-juice-fx/SKILL.md)와, 무드 색은 [`vector-graphics`](../wgf-vector-graphics/SKILL.md)/PixelForge와, flavor·톤은 [`story-architect`](../wgf-story-architect/SKILL.md)와 교차.
...
- **청취 보강(h):** 로컬 서버(`python -m http.server 8766`)로 띄워 "무드가 게임과 같은 말을 하나? SFX가 BGM에 묻히나? 모바일에서 끊기나? 루프 이음새가 튀나?"를 *직접 듣고* 점검. 가능하면 chrome-devtools/preview MCP로 콘솔 에러·Tone 초기화 확인 후 [`game-qa`](../wgf-game-qa/SKILL.md)로 점검 → **근거와 함께 보고**.
...
## 연계 / 라우팅
- **상위:** [`web-game-builder`](../wgf-web-game-builder/SKILL.md) 오케스트레이션의 사운드 설계 레인. 명시 호출은 [`commands/wgf-make-game.md`](../../commands/wgf-make-game.md), 게임 인터뷰 C5(사운드)는 [game-interview.md](../wgf-web-game-builder/reference/game-interview.md).
- **경량 레인:** [`chip-sound`](../wgf-chip-sound/SKILL.md) — 8비트 ChipAudio(T0). 아주 작은/레트로 게임이나 사운드가 부차적일 때. 이 스킬(sound-architect)이 디렉터, chip-sound가 8비트 구현 레인.
- **자매:** [`story-architect`](../wgf-story-architect/SKILL.md)(톤·무드 정합 — 무드는 톤에서 상속) · [`item-architect`](../wgf-item-architect/SKILL.md)(획득/사용 SFX) · [`level-architect`](../wgf-level-architect/SKILL.md)(구간별 강도↔적응형 인텐시티).
- **구현·연출:** [`juice-fx`](../wgf-juice-fx/SKILL.md)(SFX 동반 연출) · [`game-ui-hud`](../wgf-game-ui-hud/SKILL.md)(음소거 버튼 UI) · [`game-qa`](../wgf-game-qa/SKILL.md)(검증).
- **레퍼런스:** 색인 [reference/sound-design/INDEX.md](./reference/sound-design/INDEX.md) · 공통 원칙 [principles.md](./reference/sound-design/principles.md) · 인터뷰 [sound-interview.md](./reference/sound-interview.md) · 바이블/툴 스펙 [consistency-tools.md](./reference/sound-design/consistency-tools.md). 엔진 [engine/soundforge.js](../../engine/soundforge.js)(Tone.js v15) · 툴 [tools/lint-audio.mjs](./tools/lint-audio.mjs) · Phaser4 오디오 [audio-and-sound](../wgf-web-game-builder/reference/phaser/audio-and-sound.md).
...
- 모든 사운드는 **절차 합성**(오디오 파일 0개) → 음원 저작권·인접권 비해당, 100% CC0. vendored 라이브러리(Tone.js MIT)는 `engine/tone.LICENSE.txt`·`CREDITS.txt`에 명시. 상세는 [`ip-license-guard`](../wgf-ip-license-guard/SKILL.md).
