---
name: wgf-sprite-catalog-refresh
description: >
...
[`sprite-picker`](../wgf-sprite-picker/SKILL.md) 가 읽는 **CC0 스프라이트 소스 캐시**를 다시 만든다. 이건
...
- ❌ 평범한 스프라이트 선택/적용은 이 스킬이 아니라 [`sprite-picker`](../wgf-sprite-picker/SKILL.md).

## 산출물 (덮어쓰기 대상)
- `skills/wgf-sprite-picker/catalog/sources.json` — 소스 목록 + 안전 티어
- `skills/wgf-sprite-picker/catalog/packs.json` — 팩 인덱스 + 태그·`previewUrl`·`preview`(커밋 썸네일 경로)
- `skills/wgf-sprite-picker/catalog/thumbnails/` — 오프라인 커버 썸네일 (`prefetch.mjs` 가 cc0 팩만 벤더링)
- `skills/wgf-sprite-picker/catalog/prefetch.mjs` — 커버 썸네일 프리페치 스크립트(이 스킬이 호출, 수정은 드묾)
- 스키마: [../wgf-sprite-picker/reference/catalog-schema.md](../wgf-sprite-picker/reference/catalog-schema.md).
...
- **커버 썸네일은 `catalog/prefetch.mjs` 로 자동 벤더링.** 팩에 `previewUrl` 을 채운 뒤 실행하면 **cc0 팩만**
  ...
  ```bash
  node skills/wgf-sprite-picker/catalog/prefetch.mjs            # cc0 + previewUrl 보유 팩 전부
  node skills/wgf-sprite-picker/catalog/prefetch.mjs --only kenney --limit 8   # 일부만
  node skills/wgf-sprite-picker/catalog/prefetch.mjs --dry      # 받지 않고 대상만 출력
  ```
...
## 검증
- 두 JSON 이 [catalog-schema.md](../wgf-sprite-picker/reference/catalog-schema.md) 스키마에 맞는지(파싱·필수 필드).
...
## 연계
- 소비자: [`sprite-picker`](../wgf-sprite-picker/SKILL.md). 라이선스 게이트: [`ip-license-guard`](../wgf-ip-license-guard/SKILL.md) · 루트 `assets.json`.
