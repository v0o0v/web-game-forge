...
- 피커 UI: `node skills/sprite-picker/picker/serve.mjs` → 브라우저로 갤러리·선택 회수 동작 확인.
- 카탈로그 도구: `node skills/sprite-picker/catalog/prefetch.mjs` 등 직접 실행 + `sources.json`/`packs.json` JSON 유효성.
