...
description: "그리드 기반 퍼즐 게임을 스캐폴딩합니다 — 테트리스(Tetris), 매치3(Match-3), 2048, 뿌요뿌요류 + 보드게임 퍼즐(아줄 Azul·루미큐브 Rummikub·블로커스 Blokus·롤앤라이트류 — 드래프트·폴리오미노 패킹·세트·주사위 기입). 블록·타일을 맞추는 퍼즐/퍼즐 보드게임 요청 시 사용. puzzle, falling-block, match-3, board game, draft, polyomino, roll-and-write."
...
### 신규 하위장르 스캐폴드 (game-dna/puzzle 심화 35종 기반)
...
### 보드게임 퍼즐 스캐폴드 (game-dna/puzzle/board-* 15종 기반)
유명 퍼즐 보드게임(Azul·Patchwork·Rummikub·롤앤라이트류)의 디지털 솔로 번안. 분석·재미요소·솔로 변환 노트는 [board-draft-pattern.md](../wgf-web-game-builder/reference/game-dna/puzzle/board-draft-pattern.md) · [board-packing.md](../wgf-web-game-builder/reference/game-dna/puzzle/board-packing.md) · [board-set-rollwrite.md](../wgf-web-game-builder/reference/game-dna/puzzle/board-set-rollwrite.md) 참고. 보드모델/렌더 분리 원칙 동일. **멀티 인터랙션의 솔로 변환 필수** — 휴리스틱 AI 경쟁자(탐욕 1수 평가) / '남긴 것이 비용' 솔로 규칙 / 날짜 시드 데일리(이모지 공유) 중 최소 하나(드래프트 코어+경쟁자 부재는 안티패턴).
- **드래프트 시장 (Azul·Kingdomino·Sagrada류)**: 공유 풀 `market[]`(시드 PRNG 리필) + 개인 보드 배치 제약 검증. AI 경쟁자는 탐욕 1수 평가의 즉시 턴으로 충분('왜 가져갔는지' 읽혀야 공정). 못 놓은 자원은 감점줄에 누적(FE-DRAFT).
- **폴리오미노 패킹 (Patchwork·Blokus·Ubongo류)**: 조각=좌표 오프셋 배열, 회전=행렬 전치+행역순(Tetris 코어 재사용), 반전=열 역순. 배치 유효성(겹침·경계·접촉 규칙)+고스트 미리보기. 빈칸 수 감점 채점. 드래그&드롭 + 탭 회전(셀 스냅).
- **기입/롤앤라이트 (Ganz schön clever·Railroad Ink류)**: 시드 주사위 굴림 → 선택 → 시트 기입. 셀별 `onFill` 트리거 그래프로 보너스 기입 연쇄(FE-COMBO). 날짜 시드 데일리와 직결 — 모두 같은 굴림, 배치 실력만 비교(FE-LUCK-TAME).
- **세트 재배열 (Rummikub·SET·Qwirkle류)**: 타일=속성 튜플, 런/그룹/세트 판정은 순수 함수. Rummikub식 재구성은 테이블 스냅샷 → 임시 자유 재배열 → '제출' 시점 전체 유효성 일괄 검증 → 통과 시 확정/실패 시 원복(FE-REARRANGE). undo=스냅샷 복원.
