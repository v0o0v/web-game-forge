...
- **모바일 정합:** `instant`·`hold`는 엄지 친화. `aim`은 [`virtual-joystick`](../../../virtual-joystick/SKILL.md) 조준 스틱과, `charge`는 길게 누르기와 자연스럽다. `target`(대상 클릭)은 작은 화면에서 어려우니 자동표적/근접 우선으로 대체.
