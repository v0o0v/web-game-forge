...
> 있어야 한다. 인터뷰 A9(비주얼 슬롯)·A5(입력). HUD/UI 구현은 [`game-ui-hud`](../../../game-ui-hud/SKILL.md), 아이콘 생성은 sprite 스킬.
...
**어댑터:** 이 한 블록이 → [`sprite-forge`](../../../sprite-forge/SKILL.md)(픽셀 팔레트·프레임 시드) / [`vector-graphics`](../../../vector-graphics/SKILL.md)(베지어·글로우·재질 램프 명세) / [`sprite-picker`](../../../sprite-picker/SKILL.md)(태그·contentType·style 검색쿼리 + 대상 슬롯) 입력으로 변환된다.
...
능력 버튼은 엄지가 닿는 화면 하단 모서리에, 충분히 큰 탭타겟(≥44px)으로. 작은/먼 버튼은 오발. 이동 스틱과 능력 버튼이 안 겹치게([`virtual-joystick`](../../../virtual-joystick/SKILL.md)·MobileHarness).
