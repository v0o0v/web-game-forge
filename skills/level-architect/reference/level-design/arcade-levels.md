...
> 아케이드/단일화면(고정 화면) 게임의 난이도를 **레벨 데이터의 톱니 곡선 + 실시간 가속** 두 층으로 설계하는 처방전. 단일화면은 맵 길이로 난이도를 못 늘리므로 **시간축의 속도·밀도·페이즈·스폰**이 핵심 노브다. 공통 원칙은 [./principles.md](./principles.md), 재미요소(`FE-*`)는 [../../../web-game-builder/reference/game-dna/fun-elements.md](../../../web-game-builder/reference/game-dna/fun-elements.md), 빌드 위임은 [`arcade-classic`](#연계). 색인은 [./INDEX.md](./INDEX.md).
...
본 자료는 GMTK·GDC 공개 분석과 디자인 통념(The Art of Game Design, The Pac-Man Dossier, Level Up!, The Level Design Book)을 한글로 소화한 것이며, 법률 자문이 아니다. 상세 게이트는 [`ip-license-guard`](../../../ip-license-guard/SKILL.md).

---

## 연계

- **장르 스킬 (빌드 위임):** 이 파일의 처방은 [`arcade-classic`](../../../arcade-classic/SKILL.md) 스킬로 이어진다 — 벽돌깨기·스네이크·스택·픽스드 슈터 같은 단일화면 아케이드를 빌드할 때 `difficultyCurve()`·`phaseTimer`·`elroyMul()` 패턴을 적용한다.
...
- **제작요소 스킬:** 절차적 스프라이트는 `PixelForge`/`VectorForge`, 콤보 피치 스택·페이즈 전환음은 `ChipAudio`, 시드/결정성·점수 팝업·스크린셰이크는 `juice-fx`. 엔진 API는 [../../../web-game-builder/reference/engine-api.md](../../../web-game-builder/reference/engine-api.md), Phaser 4 색인은 [../../../web-game-builder/reference/phaser/INDEX.md](../../../web-game-builder/reference/phaser/INDEX.md).
- **레벨 빌드 메카닉:** 실제 레벨 데이터 산출(웨이브 테이블·`LEVELS` 배열 작성)은 [`level-designer`](../../../level-designer/SKILL.md)로 위임한다 — 본 파일은 *왜·어떻게* 곡선을 잡는지의 원칙, `level-designer`는 그 산출 실무다.
- **인터뷰:** 난이도 곡선·목표·페이싱 요구사항 도출은 [../level-interview.md](../level-interview.md) 플레이북. 공통 원칙 사전은 [./principles.md](./principles.md), 재미요소(`FE-*`)는 [../../../web-game-builder/reference/game-dna/fun-elements.md](../../../web-game-builder/reference/game-dna/fun-elements.md).
