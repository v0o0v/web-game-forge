...
> - **무엇을 플레이하나(재미·메카닉):** 재미요소 `FE-*` — [game-dna/fun-elements.md](../web-game-builder/reference/game-dna/fun-elements.md) (이 스킬은 그중 `FE-NARRATIVE`를 깊게 확장한다)
> - **개별 레벨의 내용·난이도 곡선:** `LD-*` — [`level-architect`](../level-architect/SKILL.md)
> - **스테이지를 잇는 진행 맵의 위상:** `MAP-*` — [`world-map-architect`](../world-map-architect/SKILL.md)
...
- **텍스트 표면 코드:** Title/막간/승패/엔딩 카드는 `this.add.text()` + tween(페이드/스케일), 환경 단서는 스프라이트 배치, bark는 floating text 풀. 카드 UI가 무거우면 [`game-ui-hud`](../game-ui-hud/SKILL.md)로.
- **분위기 구현:** 색=감정 팔레트는 [`vector-graphics`](../vector-graphics/SKILL.md)(VectorForge)/PixelForge, 무드 BGM은 [`chip-sound`](../chip-sound/SKILL.md), 연출은 [`juice-fx`](../juice-fx/SKILL.md), 환경 단서 스프라이트는 [`sprite-picker`](../sprite-picker/SKILL.md)/[`sprite-forge`](../sprite-forge/SKILL.md).
- **비트 ↔ 레벨 정합:** Beats 표의 '비트=플레이어 행동'을 [`level-architect`](../level-architect/SKILL.md)의 난이도 곡선·[`world-map-architect`](../world-map-architect/SKILL.md)의 진행과 교차 참조(예: Ten/midpoint 반전 = 곡선의 정점 직전 레벨).
...
위반은 **사람이 보게 리포트**하고 재생성한다. 가능하면 로컬 서버(`python -m http.server 8766`)로 띄워 표면 노출(인트로→막간→엔딩 흐름)을 확인하고 [`game-qa`](../game-qa/SKILL.md) 헤드리스 step 으로 점검 후 **근거와 함께 보고**한다.
...
## 연계 / 라우팅
- **상위:** [`web-game-builder`](../web-game-builder/SKILL.md) 오케스트레이션의 서사 설계 레인. 명시 호출은 [`commands/make-game.md`](../../commands/make-game.md), 게임 인터뷰는 [game-interview.md](../web-game-builder/reference/game-interview.md)(`FE-NARRATIVE` 선택 시 연계).
- **자매:** [`level-architect`](../level-architect/SKILL.md)(레벨 난이도·`LD-*`) · [`world-map-architect`](../world-map-architect/SKILL.md)(진행 맵·`MAP-*`) — Beats ↔ 곡선/진행을 교차 참조.
- **구현·분위기:** [`game-ui-hud`](../game-ui-hud/SKILL.md)(카드·메뉴) · [`vector-graphics`](../vector-graphics/SKILL.md)/[`sprite-forge`](../sprite-forge/SKILL.md)/[`sprite-picker`](../sprite-picker/SKILL.md)(색=감정·환경 단서) · [`chip-sound`](../chip-sound/SKILL.md)(무드 BGM) · [`juice-fx`](../juice-fx/SKILL.md)(연출) · [`game-qa`](../game-qa/SKILL.md)(검증).
- **레퍼런스:** 색인 [reference/story-design/INDEX.md](./reference/story-design/INDEX.md) · 공통 원칙 [principles.md](./reference/story-design/principles.md) · 인터뷰 [story-interview.md](./reference/story-interview.md) · 재미요소 [fun-elements.md](../web-game-builder/reference/game-dna/fun-elements.md)(`FE-NARRATIVE`) · Phaser4 [INDEX](../web-game-builder/reference/phaser/INDEX.md).
...
- 이름·고유명사는 STORY.md `## 8. Glossary` 에 오리지널로 정의하고, 실존 상표·작품명과 충돌하지 않게 한다. 상세는 [`ip-license-guard`](../ip-license-guard/SKILL.md).
