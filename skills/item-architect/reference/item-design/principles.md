...
- **서사 형제 바이블.** 톤·고유명사·flavor는 `games/<slug>/STORY.md`([`story-architect`](../../../story-architect/SKILL.md))를 참조해 상속한다 — 아이템 바이블이 서사를 재발명하지 않는다(`IDENT-CONSISTENT-VOICE`).
...
> 자세한 장르별 핵심 모델·티어 처방은 [scope-complexity.md](./scope-complexity.md)의 §장르 정합 표. 재미요소(`FE-BUILD`·`FE-COLLECT`·`FE-RISK-REWARD`) 조합은 [fun-elements.md](../../../web-game-builder/reference/game-dna/fun-elements.md).
