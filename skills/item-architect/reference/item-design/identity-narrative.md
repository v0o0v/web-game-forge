...
- **톤·고유명사는 발명하지 않고 [`story-architect`](../../../story-architect/SKILL.md)의 STORY.md에서 상속한다**(`IDENT-CONSISTENT-VOICE`). 아이템 바이블이 서사를 재발명하지 않는다(principles.md 공통 캐논 §0).
...
- **우리 엔진 구현(작은 웹게임):** 동사는 가벼운 것으로 충분 — 대시, 이중점프, 적 슬로우, 투사체 관통, 자원→공격 전환. 아이템 정의에 `grantsVerb: "dash"` 필드를 두고, 스탯 전용 아이템과 동사 아이템을 의도적으로 분리해 비율 관리(동사 아이템은 희소·기억 포인트). principles.md 캐논 4(`IDENT-LUDO-HARMONY`)의 "점프 게임 최고 아이템은 +5 방어가 아니라 더블점프" 근거. 재미요소 `FE-BUILD`와 직결([fun-elements.md](../../../web-game-builder/reference/game-dna/fun-elements.md)). 동사 발동 연출은 juice-fx, SFX는 chip-sound.
...
- **우리 엔진 구현(작은 웹게임):** 절차생성 레벨에서 아이템 스폰을 *의미 배치 규칙*으로 제어. 예: 부서진 소품 옆 회복템(누군가 여기서 버텼다), 같은 아이템 군집(집착의 흔적). 스폰 규칙 훅을 [level-architect](../../../level-architect/SKILL.md)(난이도 곡선)와 좌표 공유. 소품 스프라이트는 sprite-forge/vector-graphics 생성 또는 sprite-picker CC0 선택.
...
- **우리 엔진 구현(작은 웹게임):** 수집 아이템마다 (a) 작은 기계 보상 또는 (b) STORY.md lore 한 조각 중 **최소 1개를 보장**. 재미요소 `FE-COLLECT`와 직결([fun-elements.md](../../../web-game-builder/reference/game-dna/fun-elements.md)) — 컬렉션 진행 UI(game-ui-hud)와 획득 연출(juice-fx)로 시간 존중을 가시화. principles.md 안티패턴 "수집 패딩"의 가드.
...
- **정의:** flavor·이름·UI 텍스트가 한 화자/톤을 유지해야 세계가 한 작가가 쓴 듯 느껴진다. 아이템 바이블은 톤·고유명사를 **발명하지 않고 [`story-architect`](../../../story-architect/SKILL.md)의 STORY.md에서 상속**한다 — 기술 설명조차 게임의 전반 보이스에 맞춘다. principles.md 공통 캐논 11(`ITEMS-SINGLE-SOURCE`)의 서사 형제다.
...
### 자매 스킬·교차 참조
- 정체성 시각 구현: [`sprite-forge`](../../../sprite-forge/SKILL.md) / [`vector-graphics`](../../../vector-graphics/SKILL.md) / [`sprite-picker`](../../../sprite-picker/SKILL.md)(CC0) — `visual.*` 슬롯 핸드오프.
- 톤·고유명사 소스: [`story-architect`](../../../story-architect/SKILL.md) STORY.md(`TL-CANON-SINGLE-SOURCE`·`TL-CHAR-VOICE`·`ST-MIRROR-FRAME`).
- UI/연출/SFX: [`game-ui-hud`](../../../game-ui-hud/SKILL.md)(2층 툴팁·tap-to-expand) / [`juice-fx`](../../../juice-fx/SKILL.md)(획득·희소 연출) / [`chip-sound`](../../../chip-sound/SKILL.md)(청각 정체성).
- 배치 좌표: [`level-architect`](../../../level-architect/SKILL.md)(스폰 규칙·난이도 곡선).
- 라이선스: [`ip-license-guard`](../../../ip-license-guard/SKILL.md)(이름·flavor IP 차용 금지 검증).
- 재미요소: [fun-elements.md](../../../web-game-builder/reference/game-dna/fun-elements.md)(`FE-BUILD`·`FE-COLLECT`·`FE-SURPRISE`·`FE-RISK-REWARD`).
