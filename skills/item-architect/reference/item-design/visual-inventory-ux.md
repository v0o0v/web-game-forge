...
- **우리 엔진 구현(작은 웹게임):** `visual.display_px`(목표 셀 px, 기본 48) 기록. 절차생성은 그 배수(48의 2x=96)로 작업하되 다운스케일 미리보기를 함께 검수. Phaser에서 `pixelArt: true`(nearest)로 픽셀 보존. 작은 셀에서 안 보일 미세 디테일은 획득 연출(확대 팝업)에서만 노출 — juice-fx 위임([juice-fx](../../../juice-fx/SKILL.md)).
...
- **우리 엔진 구현(작은 웹게임):** `visual.category_metaphor`(무기/방어/소비/재료/열쇠/화폐 표준 메타포에 매핑). story-architect STORY.md 톤과 충돌 안 하게: 같은 "회복"도 판타지=물약병, SF=주사기, 코지=빵. 메타포 선택은 STORY.md가 제약([story-architect](../../../story-architect/SKILL.md), `IDENT-LUDO-HARMONY` 정합).
...
- **우리 엔진 구현(작은 웹게임):** `visual.material: { primary, secondary }` + `visual.palette`(램프 hex 또는 named ramp 키). sprite-forge에 재질별 표준 램프 프리셋(metal=차가운 회청 하이라이트, wood=갈색 결, gem=고채도 코어+흰 스펙큘러). vector-graphics는 같은 재질을 그라디언트 스톱으로([vector-graphics](../../../vector-graphics/SKILL.md)). sprite-picker는 `material` 키로 CC0 후보 필터([sprite-picker](../../../sprite-picker/SKILL.md)).
...
- **우리 엔진 구현(작은 웹게임):** 데이터 모델을 슬롯 배열이 아니라 `{itemId: count}` 맵으로 두면 스택·정렬이 자연스러움(`UX-LOCAL-SAVE`와 시너지). 렌더는 카테고리→희귀도/이름 정렬 기본 + "NEW" 배지. 정렬 버튼은 1개(순환) 또는 생략, 필터는 카테고리 탭으로 대체. 픽업은 즉시 카운트 증가 + juice-fx 팝. FE-COLLECT·FE-BUILD가 강한 게임은 "노동만 자동화, 조합·전시는 보존"([fun-elements.md](../../../web-game-builder/reference/game-dna/fun-elements.md)).
...
- **우리 엔진 구현(작은 웹게임):** 아이템 효과를 "즉시 체감"형으로(획득 즉시 능력↑, 다음 판 부스터). 세션 종료 시 자동 저장(quick resume). 깊은 빌드 결정은 게임 밖 메타 화면(짧게)으로 분리해 인게임 1~3분 루프 방해 금지. level-architect 1레벨 1~3분 곡선과 정렬([level-architect](../../../level-architect/SKILL.md)). FE-COLLECT·FE-BUILD는 세션 밖 메타, FE-SURPRISE(드랍)는 세션 안.
...
핵심: **본체 아이콘**(silhouette~material~palette)은 3도구가 소비하고, **등급 프레임/테두리/핍/글로우**는 game-ui-hud가 `rarity_colors` 상수로 분리 합성한다(본체에 색을 굽지 않아 재질감 보존·등급만 바뀌어도 재생성 불필요). **획득 글로우/sparkle/팝업 확대**는 juice-fx(`UX-SMALL-SIZE-TRUTH` 보완: 정적 셀에 안 보이던 디테일을 이때 노출), 등급별 SFX는 chip-sound([chip-sound](../../../chip-sound/SKILL.md)).
