...
```
이 역할색을 **HUD([`game-ui-hud`](../../wgf-game-ui-hud/SKILL.md))·능력(ability §8)·아이템(item §7)·juice 파티클**이 상속한다(`STY-PAL-SHARED-INHERIT`). 위험을 빨강으로 정했으면 데미지 파티클·위험 장판·적 강조가 모두 그 빨강을 쓴다.
...
- **아이템([`item-architect`](../../wgf-item-architect/SKILL.md) §7 스타일가이드):** 아이템 아이콘 `visual.palette` 가 master_palette 램프 참조, 등급 색이 역할색 체계와 정합.
- **능력([`ability-architect`](../../wgf-ability-architect/SKILL.md) §8 스타일가이드 헤더):** 능력 아이콘 `visual.palette`·`role_colors`·`ultimate_accent` 가 여기서 상속.
- **HUD(game-ui-hud):** 점수·체력바·강조가 `role_colors` 사용.
- **juice([`juice-fx`](../../wgf-juice-fx/SKILL.md)):** 데미지·픽업·위험 파티클이 `role_colors` 사용.
...
## 출처
- 제한 팔레트·hue-shift 셰이딩·온도 대비·의미색(2D 게임 아트·도트 통념)을 작은 웹게임·생성 도구 핸드오프용으로 정리. master_palette 단일 진실·상속 체인은 [item-architect](../../wgf-item-architect/SKILL.md) §7·[ability-architect](../../wgf-ability-architect/SKILL.md) §8 스타일가이드 헤더와 정합.
