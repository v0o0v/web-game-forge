...
- `medium:"pixel"` → [`sprite-forge`](../../wgf-sprite-forge/SKILL.md)(PixelForge 팔레트·프레임) / 외부 CC0 픽셀은 [`sprite-picker`](../../wgf-sprite-picker/SKILL.md)(태그·style 쿼리).
- `medium:"vector"` → [`vector-graphics`](../../wgf-vector-graphics/SKILL.md)(VectorForge 베지어·글로우·재질 램프) / 외부 HD CC0.
...
node skills/wgf-style-architect/tools/lint-style.mjs <file> --json     # 마지막 줄 단일 JSON
node skills/wgf-style-architect/tools/lint-style.mjs <file> --strict   # warn 도 실패
```
...
- **"Canvas 렌더러에서 룩이 멀쩡한가?"** — [`game-qa`](../../wgf-game-qa/SKILL.md) 로 WebGL 끄고 폴백 확인(`STY-MOOD-CANVAS-FALLBACK`).
...
```
- stylekit 은 **엔진 무관 코어 + 얇은 Phaser 어댑터**. Node `require('engine/stylekit.js')` 로 `load`/`palette`/`role`/`emitMasterPalette` 결정적 테스트 가능([`game-qa`](../../wgf-game-qa/SKILL.md) 호환). `Date.now` 미사용.
- 색은 master_palette 단일 진실 — 코드에 hex 중복 금지(`STYLE-SINGLE-SOURCE`).

## 출처
- 본 스펙은 [`ability-architect`](../../wgf-ability-architect/SKILL.md)의 `consistency-tools.md`(abilities.json·lint-abilities.mjs)·[`item-architect`](../../wgf-item-architect/SKILL.md) `consistency-tools.md` 컨벤션을 비주얼 스타일 도메인으로 적응시킨 것이다(출력 계약·lintConfig 임계값·단일 진실·작성/검수 분리·visual.* 어댑터 동일). style.json 스키마 계약은 `.omc/handoffs/team-plan.md` 의 확정본을 전재.
