...
WebGameForge는 **2D · 단일플레이 · 웹/모바일웹뷰 · 라이선스 안전 에셋(CC0·표기형·허용 라이선스) 또는 절차 생성**이 범위다. 아래는 많이 플레이됐지만 **핵심이 우리 범위 밖**이라 분석 대상에서 제외했다 — 단, 이들의 **개별 재미 요소**(예: 사회적 추리, 수집)는 `FE-*`로 추상화해 차용할 수 있다.
