...
- **능력 본격 설계:** 숙련 천장을 *캐릭터 능력*(콤보 연계·캔슬·정밀 발동·자원 운용)으로 만들려면 [`ability-architect`](../../../wgf-ability-architect/SKILL.md)로 라우팅한다(선딜/발동/후딜·텔레그래프·버퍼 게임필 `FEEL-*`, 콤보 링크 `COMBO-*`). 여기서 '스킬'은 캐릭터 능력 — Claude 스킬과 다르다.
...
- **아이템 본격 설계:** *무엇을 수집/획득하는가*(아이템·통화·등급·전리품·드랍 루프)를 설계하려면 [`item-architect`](../../../wgf-item-architect/SKILL.md)로 라우팅한다. 획득 동기=몰입은 가변비율·천장(pity)·등급 사다리로 만들되 도박 구조(loot box/gacha)는 차용 금지(`ECON-VARIABLE-RATIO`), 수집은 보상이나 로어로 시간을 존중(`IDENT-MEANINGFUL-COLLECT`). 단, 디폴트는 아이템 0개라 단순 코인 수집이면 이 스킬 없이도 충분.
...
- **능력 본격 설계:** 콤보가 *캐릭터 능력의 연계*(능력 A→B 캔슬·체인, 콤보가 자원/쿨다운을 생성하는 루프)면 [`ability-architect`](../../../wgf-ability-architect/SKILL.md)로 라우팅한다(콤보 윈도·캔슬 `COMBO-*`, 무한 콤보·스턴락은 `lint-abilities.mjs` 가 차단). 매치/캐스케이드 콤보는 puzzle-game, 처치 연쇄 콤보는 juice-fx/game-ui-hud 카운터.
...
- **능력 본격 설계:** 압도적 강함이 *능력의 성장·시너지*(궁극기·스킬트리·enabler+payoff로 후반 폭발)에서 나오면 [`ability-architect`](../../../wgf-ability-architect/SKILL.md)로 라우팅한다(파워예산·스케일링 캡 `BAL-*`, 시너지 `SYN-ENABLER-PAYOFF`). 곱연산 폭발·지배 능력은 `lint-abilities.mjs`/`sim-abilities.mjs` 가 차단.
...
- **아이템 본격 설계:** 빌드를 만드는 *아이템 자체*(소모품·장비·렐릭·시너지·등급·드랍)를 설계하려면 [`item-architect`](../../../wgf-item-architect/SKILL.md)로 라우팅한다(복잡도 티어 확정 → `games/<slug>/ITEMS.md` 바이블 + `items.json` → enabler/payoff·가산vs곱산 격리·진화/세트 → 아이콘 visual.* 핸드오프 → `lint-items.mjs` 밸런스 검수). 지배전략·곱연산 폭발·함정템은 이 스킬의 `SYN-*` 원칙으로 차단. 카드 UI는 game-ui-hud, 획득 연출은 juice-fx.
- **능력 본격 설계:** 빌드를 만드는 게 *캐릭터 능력/스킬*(액티브·패시브·궁극기·스킬트리·능력 시너지·콤보)이면 [`ability-architect`](../../../wgf-ability-architect/SKILL.md)로 라우팅한다(복잡도 티어 → `games/<slug>/ABILITIES.md` + `abilities.json` → enabler/payoff·콤보 연계 → `engine/abilitykit.js` 런타임 → 아이콘 핸드오프 → `lint-abilities.mjs`/`sim-abilities.mjs` 검수). 능력 부여 *아이템*과는 grantsAbility id 로 교차(아이템=그릇, 능력=정의). 여기서 '스킬'은 캐릭터 능력 — Claude 스킬과 다르다.
...
- **서사 본격 설계:** 분위기를 넘어 *이야기·캐릭터·대사·반전*까지 가려면 [`story-architect`](../../../wgf-story-architect/SKILL.md)로 라우팅한다(톤 확정 → `games/<slug>/STORY.md` 바이블 → 인트로/막간/엔딩·환경 단서·NPC 대사 + 연속성 린트). 분위기 에셋은 VectorForge(색=감정)·chip-sound(무드 BGM). 전형 vs 참신 스토리 여부를 사용자에게 묻고, 짧은 게임이므로 전복은 하나에 집중.
...
### FE-DECKCRAFT 덱 조형
- **정의:** 내 무작위 풀(덱·가방·주사위 면) 자체를 구매·제거·강화로 편집해 **확률 분포를 내가 설계**하는 재미. 뽑기 한 장은 운이지만 그 운의 분포는 내 작품 — 나쁜 카드를 정화(trash)해 덱이 점점 날카로워지는 장기 성장감이 핵심. FE-LUCK-TAME(받은 운을 길들이기)의 형제로, 이쪽은 운이 나오기 전 단계(풀 편집)를 다룬다.
- **대표 게임:** Dominion, Star Realms, The Quacks of Quedlinburg(가방 조형), Slay the Spire(디지털 계승)
- **우리 엔진 구현:** 덱=배열, 뽑기=시드 PRNG 셔플, 구매/제거=배열 편집 — 순수 데이터 로직이라 코드생성 신뢰도 최상. 런 기반 구조(전투/웨이브 사이에 카드 3택 획득·제거 상점)와 천생연분이라 topdown-shooter 런 빌드나 puzzle-game 카드 보드에 얹는다. game-ui-hud로 덱 구성 분포(색/타입 비율 바)를 항상 노출해 '내 덱이 어떤 확률 기계인지' 보이게 한다. localStorage로 런 메타 해금.
- **조합 주의:** 카드 풀이 작으면 지배 전략이 금방 고정돼 FE-BUILD 다양성이 죽는다 — 시너지 축을 2~3개 이상. 정화 수단이 없으면 덱이 비대해져 성장감이 역전(원작 Dominion의 교훈). 첫판부터 풀 전체를 열면 인지 과부하 — 점진 해금. FE-MASTERY 기록 경쟁은 시드 고정 모드에서만 공정.

### FE-AUCTION 경매·가치 평가
- **정의:** 한정 자원의 가치를 스스로 매겨 입찰·지불하는 재미. '이게 나에게 얼마짜리인가'의 평가 자체가 퍼즐이고, 너무 싸게 불러 뺏기는 아쉬움과 너무 비싸게 사는 승자의 저주 사이의 긴장이 핵심. 상대의 평가를 읽고 한 끗 위를 부르는 메타 게임이 얹힌다.
- **대표 게임:** Modern Art, Ra, For Sale, High Society, Power Grid
- **우리 엔진 구현:** 솔로 번안이 관건 — (a) 평가 휴리스틱이 읽히는 AI 입찰자(아이템 가치를 공개 기준으로 산정+약간의 변주, '왜 그 가격에 멈췄는지' 납득되게), (b) 비대인 경매: 시간·행동·체력 같은 내 자원을 화폐로 내는 '시스템 상대 입찰'(Patchwork 시간 트랙 친족), (c) 한 번 패스하면 끝인 1회 입찰(Ra식)로 결단 압박. game-ui-hud로 내 잔액·시세 이력을 노출해 가치 판단 근거를 준다.
- **조합 주의:** 다인 호가 경매의 심리전은 솔로에서 약해진다 — AI 입찰만으로는 긴장이 안 살면 '1회 입찰·강제 종료(Ra)' 같은 구조 긴장으로 대체하라. 시세 정보가 없으면 가치 평가가 찍기가 된다(FE-FAIRNESS 훼손) — 이력·기준가를 반드시 노출. FE-ESCALATION 실시간 카운트다운 경매는 사고 시간을 죽인다.

### FE-BLUFF 심리전·은닉
- **정의:** 내 정보(패·역할·의도)를 숨기고 거짓을 섞어 상대의 판단을 오염시키며, 동시에 상대의 거짓을 베팅·도전으로 읽어내는 재미. '들킬까'의 스릴과 '잡아냈다'의 쾌감이 양날.
- **대표 게임:** Coup, Skull, Liar's Dice(Perudo), Cockroach Poker, Sheriff of Nottingham
- **우리 엔진 구현:** 다인 사회성 코어는 솔로 ❌에 가깝다 — 직역하지 말고 두 갈래로 추출: (a) **AI 텔 관찰**: AI 상대에게 의도적 행동 패턴(베팅 습관·지연·표정 아이콘)을 심고 플레이어가 여러 판에 걸쳐 학습하게 한다(패턴은 판마다 시드 변주), (b) **확률 추론 퍼즐화**: Liar's Dice의 호가를 '남은 주사위 분포 기대값 추정' 문제로 노출해 FE-DEDUCTION 쪽으로 끌어온다. 내 블러프는 'AI가 내 과거 행동 통계에 반응'하는 구조로 성립시킨다. game-ui-hud로 확률 힌트(초보 모드)·노출 정보를 구분 표시.
- **조합 주의:** AI가 거짓말을 '완전 랜덤'으로 하면 읽을 텔이 없어 도박이 된다(FE-FAIRNESS 붕괴) — 반드시 학습 가능한 패턴+변주로. 인간 대체를 약속하지 말 것: 솔로 블러프는 '심리전'이 아니라 '패턴 해독+확률 베팅'임을 게임이 솔직하게 드러내야 실망이 없다. FE-DEDUCTION 순수 연역과 섞으면 '추론하면 풀린다' 결정성이 흔들리니 모드 분리.

### FE-MEMORY 기억
- **정의:** 가려진 정보의 위치·순서·변화를 머릿속에 유지했다가 정확히 꺼내 맞추는 재미. 위치 기억(같은 그림 쌍), 누적 시퀀스(점점 길어지는 패턴 따라하기), 변화 추적(방금 뭐가 바뀌었나)의 세 갈래. FE-HIDDEN-INFO(단서로 추론)와 형제지만 이쪽은 추론이 아니라 **암기**가 실력.
- **대표 게임:** Memory(Concentration), Simon, Ghost Blitz(조건 반전 인출), Hanabi(내 패 추적)
- **우리 엔진 구현:** 상태(진짜 값)와 표시(뒤집힘/가려짐)를 분리 저장하고 reveal 시점만 노출 — puzzle-game 보드모델로 구현이 가장 쉬운 축. 시퀀스 기억은 입력 타임라인 배열 비교. 난도 곡선은 칸 수·시퀀스 길이·노출 시간·방해 요소(셔플·유사 그림)로 조절(level-designer 파라미터). juice-fx는 정답 연쇄에 단계 상승 피드백, ChipAudio 음계로 시퀀스를 청각 보조(Simon식 멀티모달 암기).
- **조합 주의:** 노출 시간이 짧고 항목이 많으면 실력이 아니라 한계 시험이 된다 — 인지 한계(4±1 청크)를 존중해 난도를 올릴 것. FE-ESCALATION과 결합은 자연스럽지만 가속이 인간 단기기억 한계를 넘으면 불공정. 색만으로 구분하면 색약 접근성 문제 — 모양·소리 채널 병행. 순수 암기만 길면 단조 — FE-RISK-REWARD(더 외울까 지금 답할까) 같은 선택을 얹어라.
...
### 덱조형 런 슈터 (Deckcraft Run Shooter)
- **레시피:** 코어 루프: topdown-shooter 런 + FE-DECKCRAFT + FE-BUILD + FE-RISK-REWARD
- **예시 신작:** 웨이브 사이에 카드 3택으로 '발사 덱'에 카드를 넣거나 빼고, 매 발사가 덱 맨 위 카드로 결정되는 — 덱 구성이 곧 무기 성격인 덱 조형 트윈스틱 슈터.
- **왜 재밌나:** 발사=뽑기라서 덱 편집이 즉시 체감되는 확률 설계(FE-DECKCRAFT)가 되고, 정화로 날카로워지는 시너지 성장(FE-BUILD)과 '강하지만 무거운 카드를 넣을까 얇고 안정적으로 갈까'의 저울질(FE-RISK-REWARD)이 런마다 다른 빌드를 만든다. 덱=배열+시드 셔플의 순수 데이터 로직이라 신뢰도 높고, 카드 3택 화면은 웨이브 사이 자연 완급(FE-FLOW 안티패턴 회피). game-ui-hud로 덱 분포 바 상시 노출.

### 시스템 경매 타이쿤 (Solo Auction Tycoon)
- **레시피:** 코어 루프: puzzle-game 턴제 보드 + FE-AUCTION + FE-CONSTRAINT + FE-OPTIMIZE
- **예시 신작:** 라운드마다 공개되는 매물(작품·부동산)에 평가 습관이 읽히는 AI 입찰자 2~3인과 1회 입찰로 겨뤄 사들이고, 한정 자본 안에서 시세 차익을 최적화하는 경매 타이쿤.
- **왜 재밌나:** '이게 나에게 얼마짜리인가'의 가치 평가(FE-AUCTION)가 자본 제약(FE-CONSTRAINT) 위에서 무게를 갖고, 시세 이력 기반 차익 최적화(FE-OPTIMIZE)가 숙련 천장을 만든다. Ra식 1회 입찰·강제 종료로 다인 호가 없이도 결단 압박이 산다. AI 평가 기준 공개+시세 이력 HUD로 공정성(FE-FAIRNESS) 확보, 날짜 시드 데일리로 같은 매물 라인업 경쟁.

### 텔 읽기 듀얼 (Tell-Reading Duel)
- **레시피:** 코어 루프: puzzle-game 턴제 대결 + FE-BLUFF + FE-DEDUCTION + FE-TENSION
- **예시 신작:** 가려진 주사위 수량을 호가하는 AI와의 라이어 다이스 듀얼 — AI의 베팅 습관(텔)을 여러 판에 걸쳐 학습하고, 남은 분포 기대값을 추론해 도전/수용을 판단하는 블러프 듀얼.
- **왜 재밌나:** 다인 심리전을 '패턴 해독+확률 추론'으로 정직하게 솔로화한다 — AI 텔은 학습 가능한 패턴+시드 변주(FE-BLUFF), 호가 판단은 기대값 연역(FE-DEDUCTION), 도전 선언 순간의 까뒤집기(FE-TENSION)가 한 판의 절정. 초보 모드 확률 힌트 토글, 판마다 짧아 FE-JUST-ONE-MORE 친화.

### 시퀀스 폭주 암기 (Runaway Sequence Memory)
- **레시피:** 코어 루프: arcade-classic + FE-MEMORY + FE-ESCALATION + FE-JUST-ONE-MORE
- **예시 신작:** 한 단계마다 한 음씩 길어지는 빛·소리 시퀀스를 따라 입력하되, 단계가 오를수록 방해 요소(색 반전·버튼 위치 셔플)가 하나씩 끼어드는 암기 서바이벌.
- **왜 재밌나:** 누적 시퀀스 암기(FE-MEMORY)에 점증 압박(FE-ESCALATION)이 자연 결합하고, 실패 즉시 재시작(FE-JUST-ONE-MORE)이 '한 단계만 더'를 만든다. 음계+색+위치의 멀티모달 암기 보조(ChipAudio)로 인지 한계를 존중하면서 천장을 올린다. 입력 타임라인 배열 비교라 구현 신뢰도 최상, localStorage 베스트 단계 기록.
...
- **다인 사회성 코어 + 솔로 직역:** 협상(Chinatown류)·정체 은닉(마피아류)·제한 소통(Hanabi류)·파티 연상(Codenames류)의 재미는 **사람 사이에서만** 발생한다. AI를 사람 자리에 그대로 끼우면 협상은 메뉴 선택이, 블러프는 난수가, 연상 단서는 사전 룩업이 되어 코어가 증발한다. 직역하지 말고 요소를 추출·변환하라 — 블러프→AI 텔 패턴 해독+확률 추론(FE-BLUFF§ 참조), 단서 경제→횟수 제약 퍼즐(FE-CONSTRAINT), 위기 분담→다중 유닛 저글링(FE-PLANNING). 변환 불가능한 코어는 ❌로 솔직히 표기하고 다른 코어를 찾는다.
- **실물 민첩 코어 + 정밀 물리 직역:** Jenga·Crokinole류 실물 게임의 손맛은 마찰·무게중심·손끝의 아날로그 피드백에서 온다. 이를 Matter 정밀 강체로 1:1 재현하려 하면 물리 신뢰도 ⚠️에 모바일 터치 정밀도 한계가 겹쳐 '조작감 나쁜 시뮬레이터'가 된다. '타이밍 탭 추상화'(흔들림 게이지를 정점에서 멈추기, 파워·각도 2탭 발사)나 단순 임펄스 근사로 번안해 긴장(FE-TENSION)과 타이밍(FE-TIMING)의 정수만 가져오라.
...
- FE-DECKCRAFT / FE-AUCTION / FE-BLUFF / FE-MEMORY (보드게임 계열) → 보드게임 아틀라스 [board/INDEX.md](./board/INDEX.md)의 해당 클러스터 + game-ui-hud(덱 분포·시세 이력·확률 힌트)
