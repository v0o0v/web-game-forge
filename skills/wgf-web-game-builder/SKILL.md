...
- **먼저 아트 출처를 가른다(결정 게이트).** 비주얼은 재미의 핵심이므로, 인터뷰(C4)에서 **실제 에셋을
  ...
  스프라이트를 *선택*하려는 기색이면 [`sprite-picker`](../wgf-sprite-picker/SKILL.md) 로 위임한다(CC0 카탈로그·
...
### 4) 사운드 — 두 레인 (게임당 하나)
사운드는 **디렉터 스킬 [`sound-architect`](../wgf-sound-architect/SKILL.md) 가 무드 인터뷰로 설계**한다(make-game 사운드 게이트). 엔진은 둘 중 하나:
...
- 검수: `node skills/wgf-sound-architect/tools/lint-audio.mjs games/<slug>/audio.json`.

**B. ChipAudio (8비트 경량 · `engine/audio.js`)** — 아주 작은/레트로 게임(T0, [`chip-sound`](../wgf-chip-sound/SKILL.md)).
