...
> 실제 이미지 생성은 [`sprite-forge`](../sprite-forge/SKILL.md)·[`vector-graphics`](../vector-graphics/SKILL.md)·
> [`sprite-picker`](../sprite-picker/SKILL.md)의 소관이다. style-architect는 **그 위에서 "이 게임은 어떻게 보이는가"를
...
> - **톤·이야기·캐릭터·대사:** [`story-architect`](../story-architect/SKILL.md) (STYLE.md는 STORY.md 톤을 상속한다)
> - **습득·사용하는 모든 것(아이템):** [`item-architect`](../item-architect/SKILL.md) (§7 비주얼이 master_palette를 상속)
> - **캐릭터가 할 수 있는 행동(능력):** [`ability-architect`](../ability-architect/SKILL.md) (§8 비주얼이 master_palette를 상속)
> - **개별 레벨·진행 맵:** [`level-architect`](../level-architect/SKILL.md) · [`world-map-architect`](../world-map-architect/SKILL.md)
> - **그 위에 입히는 소리:** [`sound-architect`](../sound-architect/SKILL.md)
> - **실제 이미지를 *생성*하는 도구:** [`sprite-forge`](../sprite-forge/SKILL.md)/[`vector-graphics`](../vector-graphics/SKILL.md)/[`sprite-picker`](../sprite-picker/SKILL.md) — **이들은 style.json을 입력으로 받는다.**
...
8. **과설계 금지 · IP 안전.** 한 게임 한 매체·작은 팔레트(색 수 상한), 죽은 색·저대비·과채도 차단. **상용 게임의 에셋·이름·시그니처 룩을 복제하지 않는다** — 스타일/무드/기법만 차용해 오리지널로 재구성한다(`lintConfig.ip_redwords`로 기계 점검, [`ip-license-guard`](../ip-license-guard/SKILL.md)).
...
- **라이브 웹 리서치(WebSearch/WebFetch):** 내장 원칙은 광범위 웹 리서치를 작은 웹게임용으로 정리한 1차 라이브러리이니 **항상 먼저 적용**. 그 위에, 특정 무드·장르의 아트 *결*이 필요하면 그 관습·기법을 능동 리서치해 보강한다. **IP 안전 가드**: 팔레트 무드·기법만 차용, 고유 에셋·이름·시그니처는 오리지널 재구성([`ip-license-guard`](../ip-license-guard/SKILL.md)).
...
- **생성기 팔레트 상속:** [`sprite-forge`](../sprite-forge/SKILL.md)(PixelForge)·[`vector-graphics`](../vector-graphics/SKILL.md)(VectorForge)·[`sprite-picker`](../sprite-picker/SKILL.md)가 `StyleKit.palette`/`palette.master.json`·셰이딩·광원 상수를 입력으로 받아 같은 룩으로 생성한다.
- **타일·HUD·juice 색 상속:** [`level-designer`](../level-designer/SKILL.md)(tiled 타일색)·[`game-ui-hud`](../game-ui-hud/SKILL.md)(role_colors HUD)·[`juice-fx`](../juice-fx/SKILL.md)(파티클 색)가 `role_colors`·master_palette를 상속해 게임 전체가 한 스타일을 유지한다.
- **라이팅·포스트FX 무드:** `StyleKit.applyLighting(scene, STYLE)`→[`lighting-mood`](../lighting-mood/SKILL.md)(engine/lightingkit.js), `StyleKit.applyPostFX(camera, STYLE)`→[`screen-fx`](../screen-fx/SKILL.md)(engine/screenfx.js). **WebGL 불가 시 `StyleKit.applyCanvasFallback(scene, STYLE)`**로 틴트 오버레이/사전셰이딩(`STY-MOOD-CANVAS-FALLBACK`, R4).
...
- 위반은 **사람이 보게 리포트**하고 재생성한다. 가능하면 로컬 서버로 띄워 룩·무드·폴백을 [`game-qa`](../game-qa/SKILL.md)로 점검 후 **근거와 함께 보고**한다.
...
- **있으면 상속:** `games/<slug>/style.json`(또는 `assets/palette.master.json`)이 있으면 [`item-architect`](../item-architect/SKILL.md) §7 비주얼 헤더와 [`ability-architect`](../ability-architect/SKILL.md) §8 스타일가이드 헤더의 `master_palette`는 **이 파일을 상속**한다 — 아이템·능력 아이콘이 게임 전체 룩과 자동으로 응집한다.
...
## 연계 / 라우팅
- **상위:** [`web-game-builder`](../web-game-builder/SKILL.md) 오케스트레이션의 아트 디렉션 레인. 명시 호출은 [`commands/make-game.md`](../../commands/make-game.md), 게임 인터뷰는 [game-interview.md](../web-game-builder/reference/game-interview.md).
- **자매 디렉터:** [`story-architect`](../story-architect/SKILL.md)(톤↔무드 정합, STYLE는 STORY 상속) · [`item-architect`](../item-architect/SKILL.md)(§7 master_palette 상속) · [`ability-architect`](../ability-architect/SKILL.md)(§8 master_palette 상속) · [`level-architect`](../level-architect/SKILL.md)/[`world-map-architect`](../world-map-architect/SKILL.md)(진행) · [`sound-architect`](../sound-architect/SKILL.md)(청각 무드 정합).
- **이미지 생성기(style.json 상속):** [`sprite-forge`](../sprite-forge/SKILL.md)(PixelForge 팔레트) · [`vector-graphics`](../vector-graphics/SKILL.md)(VectorForge 재질 램프) · [`sprite-picker`](../sprite-picker/SKILL.md)(CC0 검색 style 필터).
- **무드·UI 구현:** [`lighting-mood`](../lighting-mood/SKILL.md)(라이팅) · [`screen-fx`](../screen-fx/SKILL.md)(포스트FX) · [`level-designer`](../level-designer/SKILL.md)(타일 색) · [`game-ui-hud`](../game-ui-hud/SKILL.md)(role_colors HUD) · [`juice-fx`](../juice-fx/SKILL.md)(파티클 색) · [`game-qa`](../game-qa/SKILL.md)(룩·무드·폴백 검증).
...
- 무드 id·고유명사는 STYLE.md 와 STORY.md `## 8. Glossary`에 오리지널로 정의한다. 모든 에셋은 CC0/절차생성(PixelForge·VectorForge) 또는 CC0(sprite-picker). `style.json` `lintConfig.ip_redwords`로 상용 IP 이름이 mood/slug에 누출되지 않는지 기계 점검한다. 상세는 [`ip-license-guard`](../ip-license-guard/SKILL.md).
