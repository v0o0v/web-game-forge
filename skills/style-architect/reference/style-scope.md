...
스타일은 *과설계가 가장 쉬운 영역*이다. 디폴트는 **T0(제한 팔레트 하나)** — 색만 고정하고 폼·셰이딩은 엔진 기본([`pixelforge`](../../sprite-forge/SKILL.md)·[`vectorforge`](../../vector-graphics/SKILL.md) 기본 셰이딩)에 맡긴다. 한 칸씩 정당화하며 올린다(`STY-SCOPE-LADDER`). "예쁘게 해줘"는 보통 *일관된 제한 팔레트 + 실루엣*만으로 80% 달성된다 — 라이팅/포스트FX(T3)는 무드가 게임의 정체성일 때만.
...
스타일의 두 번째 갈림길은 **매체**다 — `medium: "pixel"`(레트로·도트·해상도 제한) vs `medium: "vector"`(매끈·그라디언트·글로우·곡선). 이게 어떤 생성 도구로 라우팅될지(픽셀→[`sprite-forge`](../../sprite-forge/SKILL.md), 벡터→[`vector-graphics`](../../vector-graphics/SKILL.md))와 셰이딩 어휘를 통째로 바꾼다.
...
## 출처
- 제한 팔레트·매체 구분·복잡도 게이트(인디 2D 아트·도트 통념)를 작은 웹게임용으로 정리. [ability-design/scope-complexity.md](../../ability-architect/reference/ability-design/scope-complexity.md)의 복잡도 사다리·디폴트 최소·한 게임 한 핵심모델 패턴을 비주얼 스타일 도메인으로 적응.
