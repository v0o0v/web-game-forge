...
> 컬러 그레이딩). 여기서 style.json `lighting`/`postfx` 를 [lighting-mood](../../lighting-mood/SKILL.md)([`engine/lightingkit.js`](../../../engine/lightingkit.js))·
> [screen-fx](../../screen-fx/SKILL.md)([`engine/screenfx.js`](../../../engine/screenfx.js)) 프리셋으로 매핑하고, **R4 Canvas-safe 폴백**을 명시한다.
...
> 검증: [`game-qa`](../../game-qa/SKILL.md) 로 Canvas 렌더러 강제 후 룩이 깨지지 않는지 확인. lint-style.mjs 는 `lighting`/`postfx` 가 있는데 `canvas_fallback` 이 비면 warn.
