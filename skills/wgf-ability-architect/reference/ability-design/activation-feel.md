...
> 숙련 가능해진다. 인터뷰 A5. juice/연출은 [`juice-fx`](../../../wgf-juice-fx/SKILL.md)·사운드는 [`sound-architect`](../../../wgf-sound-architect/SKILL.md)가 구현,
...
타격 순간 1~5프레임(≤80ms) 정지로 "맞았다"의 무게를 준다. 강한 능력일수록 길게(단 80ms 초과는 흐름 끊김, `FE-JUICE` 주의). 스크린셰이크·파티클·SFX와 묶어 [`juice-fx`](../../../wgf-juice-fx/SKILL.md)가 구현.
...
능력 입력을 엄지에 맞춘다: 탭(instant), 길게(charge/hold), 조준 스틱(aim, [`virtual-joystick`](../../../wgf-virtual-joystick/SKILL.md)), 스와이프(방향 능력). 작은 버튼·먼 위치는 오발을 부른다 — 엄지 영역(`UX-THUMB-ZONE`)에 큰 탭타겟.
...
모든 능력에 풀 연출을 깔면 감각이 마비되고 흐름을 가린다. 시그니처·궁극에 강한 연출, 기본기엔 가벼운 피드백. juice는 *정보*(무엇이 일어났나)를 먼저, *멋*은 그다음([`juice-fx`](../../../wgf-juice-fx/SKILL.md) `FE-JUICE` 주의).
