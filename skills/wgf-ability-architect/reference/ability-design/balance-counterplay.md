...
## 검수 워크플로 (작성과 분리 — 필수)
1. **정적(필수):** `node skills/wgf-ability-architect/tools/lint-abilities.mjs games/<slug>/abilities.json` — schema·dead-skill·dominant·mult-explode·resource·cooldown·synergy·tree·gate-softlock·combo-loop·input-budget.
2. **동적(복잡 킷):** `node skills/wgf-ability-architect/tools/sim-abilities.mjs <file> --build a,b,c --duration 60` — DPS·자원 고갈률·능력 점유율·지배 능력.
