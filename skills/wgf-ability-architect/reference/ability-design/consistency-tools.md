...
node skills/wgf-ability-architect/tools/lint-abilities.mjs <file> --json     # 마지막 줄 단일 JSON
node skills/wgf-ability-architect/tools/lint-abilities.mjs <file> --strict   # warn 도 실패
```
...
```bash
node skills/wgf-ability-architect/tools/sim-abilities.mjs games/<slug>/abilities.json --build a,b,c --duration 60 --dt 0.05
```
...
- 헤드리스 검증: Node 에서 `require('engine/abilitykit.js')` 로 `use/tick/learn` 결정적 테스트 가능([`game-qa`](../../../wgf-game-qa/SKILL.md) step 하니스 호환).

## 출처
- 본 스펙은 [`item-architect`](../../../wgf-item-architect/SKILL.md)의 `consistency-tools.md`(items.json·lint-items.mjs) 컨벤션을 능력 도메인으로 적응시킨 것이다(출력 계약·balanceConfig 임계값·단일 진실·작성/검수 분리 동일).
