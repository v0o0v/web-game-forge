...
> *코드로 즉석 제작*은 [`sprite-forge`](../wgf-sprite-forge/SKILL.md)(픽셀, PixelForge) ·
> [`vector-graphics`](../wgf-vector-graphics/SKILL.md)(스무스, VectorForge) 가 한다 — 이 스킬이 절차 생성을 그쪽에 위임한다.
> 카탈로그 갱신은 [`sprite-catalog-refresh`](../wgf-sprite-catalog-refresh/SKILL.md).
...
4. **캐싱 우선.** 매번 웹을 뒤지지 않는다 — 미리 조사·검증된 `catalog/` 를 읽는다. 외부 재조사는 사용자가
   [`sprite-catalog-refresh`](../wgf-sprite-catalog-refresh/SKILL.md) 로 명시 요청할 때만.
5. **CC0/IP-safe.** `cc0` 우선, 표기 필요는 `CREDITS.txt` 약속 시, 그 외는 게이트에서 막는다([`ip-license-guard`](../wgf-ip-license-guard/SKILL.md)).
...
- `skills/wgf-sprite-picker/picker/data.js` 로 주입(`data.example.js` 템플릿). 스키마: [reference/catalog-schema.md](./reference/catalog-schema.md).
- **커밋 썸네일 미리 받기(빠른 실행):** 카탈로그의 CC0 팩 커버는 [`sprite-catalog-refresh`](../wgf-sprite-catalog-refresh/SKILL.md)
  의 `catalog/prefetch.mjs` 로 미리 받아 `catalog/thumbnails/` 에 둔다. 평소엔 이 캐시만 쓴다.
- **컴패니언 서버로 서빙(자동 회수+풀뷰용):** `node skills/wgf-sprite-picker/picker/serve.mjs` (background) →
...
1. **팩 다운로드:**
   ```
   node skills/wgf-sprite-picker/catalog/fetch-pack.mjs --pack <packId> [--out <dir>] [--dry]
   ```
...
2. **분석 및 library.json 갱신:**
   ```
   node skills/wgf-sprite-picker/catalog/analyze-pack.mjs --pack <packId> [--lib <dir>]
   ```
...
- 절차 생성 → [`sprite-forge`](../wgf-sprite-forge/SKILL.md)/[`vector-graphics`](../wgf-vector-graphics/SKILL.md) 위임.
...
- 로컬 서버로 띄워 스프라이트·애니 렌더와 콘솔 에러 0 확인, 스타일 일관성·모바일 부담 점검,
  최종 라이선스 게이트는 [`ip-license-guard`](../wgf-ip-license-guard/SKILL.md). 결과를 근거와 함께 보고.

## 연계 / 라우팅
- **상위:** [`web-game-builder`](../wgf-web-game-builder/SKILL.md) 오케스트레이션의 아트 소싱 레인. 게임 생성 중 아트 결정 게이트로 호출.
- **위임 대상(절차 생성):** [`sprite-forge`](../wgf-sprite-forge/SKILL.md)(픽셀) · [`vector-graphics`](../wgf-vector-graphics/SKILL.md)(스무스).
- **갱신:** [`sprite-catalog-refresh`](../wgf-sprite-catalog-refresh/SKILL.md)(카탈로그 웹 재조사·캐싱).
- **품질:** [`ip-license-guard`](../wgf-ip-license-guard/SKILL.md)(라이선스) · [`perf-60fps`](../wgf-perf-60fps/SKILL.md)(텍스처 부담) · [`game-ui-hud`](../wgf-game-ui-hud/SKILL.md)(UI 아트).
- **레퍼런스:** 데이터 스키마 [catalog-schema.md](./reference/catalog-schema.md) · 피커 프로토콜 [picker-protocol.md](./reference/picker-protocol.md)
  ...
  · Phaser4 로딩 [loading-assets](../wgf-web-game-builder/reference/phaser/loading-assets.md) · [animations](../wgf-web-game-builder/reference/phaser/animations.md).

## IP 안전
- 카탈로그는 `cc0`/`permissive-attribution` 위주로 큐레이션하며 `safetyTier` 로 등급을 단다. `mixed-per-item`·`avoid`
  는 적용 전 항목별 확인 필수.
- 닌텐도 등 상용 IP 리핑 소스는 카탈로그에 넣지 않는다. 보호된 이름·시그니처 조합 금지(상세 [`ip-license-guard`](../wgf-ip-license-guard/SKILL.md)).
