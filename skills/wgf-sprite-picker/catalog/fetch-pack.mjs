 ...
 *
 * 실행:
 *   node skills/wgf-sprite-picker/catalog/fetch-pack.mjs --pack <packId>
 *   node skills/wgf-sprite-picker/catalog/fetch-pack.mjs --pack kenney-tiny-dungeon --out assets-library
 *   node skills/wgf-sprite-picker/catalog/fetch-pack.mjs --pack <packId> --dry   # 해석만, 받지 않음
 *
...
(async function main() {
  ...
  if (opt.manual) {
    ...
    console.log('다음 단계(라이선스 필수): node skills/wgf-sprite-picker/catalog/analyze-pack.mjs --pack ' + opt.id + ' --license <CC0-1.0|CC-BY-4.0|...> --name "<표시명>" --source <itch|local|...>');
  ...
  allFiles.slice(0, 40).forEach(function (f) { console.log('  ·', f); });
  if (allFiles.length > 40) console.log('  · ... (+' + (allFiles.length - 40) + ' more)');
  console.log('다음 단계: node skills/wgf-sprite-picker/catalog/analyze-pack.mjs --pack ' + pack.id);
