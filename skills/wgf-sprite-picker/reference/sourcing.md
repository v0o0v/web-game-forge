...
2. **온디맨드 다운로드(피커 다운로드 버튼 → 큐 → Claude 실행).** 피커의 카탈로그 카드에 "⬇ 다운로드"
   ...
   **fetch-pack.mjs — 팩 다운로드:**
   ```
   node skills/wgf-sprite-picker/catalog/fetch-pack.mjs --pack <packId> [--out <dir=assets-library>] [--dry]
   ```
   ...
   **off-catalog 수동 핸드오프 (itch.io 등 게이트·미카탈로그 팩).** itch.io 는 매일 새 에셋이 올라오고
   ...
   2. `fetch-pack.mjs` 수동 모드로 `raw/` 에 푼다(카탈로그·리졸버·CC0-by-packId 게이트 우회):
      ```
      node skills/wgf-sprite-picker/catalog/fetch-pack.mjs --id <slug> --zip <로컬ZIP경로>
      node skills/wgf-sprite-picker/catalog/fetch-pack.mjs --id <slug> --url <직접파일URL>
      ```
   3. `analyze-pack.mjs` 로 분석하되, **off-catalog 는 라이선스를 가정하지 않으므로 `--license` 필수**
      ...
      ```
      node skills/wgf-sprite-picker/catalog/analyze-pack.mjs --pack <slug> --license <CC0-1.0|CC-BY-4.0|...> \
...
> [loading-assets](../../wgf-web-game-builder/reference/phaser/loading-assets.md) ·
> [sprites-and-images](../../wgf-web-game-builder/reference/phaser/sprites-and-images.md) ·
> [animations](../../wgf-web-game-builder/reference/phaser/animations.md) 참고.
...
- 픽셀아트 → [`sprite-forge`](../../wgf-sprite-forge/SKILL.md) (`PixelForge.bake` 문자 그리드).
- 미려한 스무스/벡터 → [`vector-graphics`](../../wgf-vector-graphics/SKILL.md) (`VectorForge.bake` drawFn).
...
- 스타일 일관성(한 게임 한 스타일), 모바일에서 텍스처 크기/필레이트 과부하 없는지([`perf-60fps`](../../wgf-perf-60fps/SKILL.md)).
- 최종 라이선스 게이트는 [`ip-license-guard`](../../wgf-ip-license-guard/SKILL.md) 로 점검.
